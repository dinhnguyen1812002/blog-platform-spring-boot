package com.Nguyen.blogplatform.domain.user.infrastructure.repository;



import com.Nguyen.blogplatform.domain.analytics.dto.MonthlyStatDTO;
import com.Nguyen.blogplatform.domain.analytics.dto.TopAuthorDTO;
import com.Nguyen.blogplatform.domain.post.domain.model.Post;
import com.Nguyen.blogplatform.domain.user.domain.model.Role;
import com.Nguyen.blogplatform.domain.user.domain.model.User;
import com.Nguyen.blogplatform.domain.user.dto.TopUserResponse;
import com.Nguyen.blogplatform.shared.enums.ERole;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import org.springframework.stereotype.Repository;

import java.time.LocalDateTime;
import java.util.List;
import java.util.Optional;

@Repository
public interface UserRepository extends JpaRepository<User, String> {
    Optional<User> findByUsername(String username);
    @Query("SELECT u FROM User u JOIN FETCH u.roles WHERE u.username = :username")
    Optional<User> findByUsernameWithRoles(@Param("username") String username);
    Optional<User> findByEmail(String email);
    @Query("SELECT u FROM User u JOIN FETCH u.roles WHERE u.email = :email")
    Optional<User> findByEmailWithRoles(@Param("email") String email);

    Optional<User> findBySlug(String slug);
    Boolean existsBySlug(String slug);

    Boolean existsByUsername(String username);

    Boolean existsByEmail(String email);
    User findByResetToken(String resetToken);

    // Role-related queries
    Page<User> findByRolesContaining(Role role, Pageable pageable);
    List<User> findByRolesContaining(Role role);
    Long countByRolesContaining(Role role);

    @Query("SELECT u FROM User u JOIN u.roles r WHERE r.name = :roleName")
    List<User> findByRoleName(@Param("roleName") com.Nguyen.blogplatform.shared.enums.ERole roleName);

    @Query("SELECT u FROM User u JOIN u.roles r WHERE r.name = :roleName")
    Page<User> findByRoleName(@Param("roleName") com.Nguyen.blogplatform.shared.enums.ERole roleName, Pageable pageable);
    @Query("SELECT u FROM User u JOIN FETCH u.roles WHERE u.id = :id")
    Optional<User> findByIdWithRoles(@Param("id") String id);

    long countByCreatedAtBetween(LocalDateTime start, LocalDateTime end);

//    Page<User> getUserPostByPost();

//    @Query("SELECT new com.Nguyen.blogplatform.domain.user.dto.TopUserResponse(u.id, u.username, u.email, u.avatar, COUNT(p)) " +
//           "FROM User u LEFT JOIN u.posts p " +
//           "GROUP BY u.id, u.username, u.email, u.avatar " +
//           "ORDER BY COUNT(p) DESC")
//    java.util.List<com.Nguyen.blogplatform.domain.user.dto.TopUserResponse> findTopUsersByPostCount(org.springframework.data.domain.Pageable pageable);
//



    List<User> findAllByOrderByCreatedAtDesc(Pageable pageable);
    boolean existsByRoles_Name(ERole name);

    @Query("SELECT COUNT(u) FROM User u WHERE u.createdAt >= :since")
    Long countActiveUsers(@Param("since") LocalDateTime since);

    @Query("SELECT new com.Nguyen.blogplatform.domain.analytics.dto.MonthlyStatDTO(" +
            "YEAR(u.createdAt), MONTH(u.createdAt), COUNT(u)) " +
            "FROM User u " +
            "WHERE YEAR(u.createdAt) = :year " +
            "GROUP BY YEAR(u.createdAt), MONTH(u.createdAt) " +
            "ORDER BY MONTH(u.createdAt) ASC")
    List<com.Nguyen.blogplatform.domain.analytics.dto.MonthlyStatDTO> countNewUsersPerMonth(@Param("year") int year);

    @Query("SELECT NEW com.Nguyen.blogplatform.domain.analytics.dto.TopAuthorDTO(" +
           "u.id, u.username, " +
           "COALESCE(SUM(p.viewCount), 0L), " +
           "COALESCE(SUM(SIZE(p.likes)), 0L), " +
           "COUNT(p)) " +
           "FROM User u LEFT JOIN Post p ON p.author = u " +
           "GROUP BY u.id, u.username " +
           "ORDER BY (COALESCE(SUM(p.viewCount), 0L) + COALESCE(SUM(SIZE(p.likes)), 0L)) DESC")
    List<com.Nguyen.blogplatform.domain.analytics.dto.TopAuthorDTO> findTopAuthorsByEngagement(Pageable pageable);
}