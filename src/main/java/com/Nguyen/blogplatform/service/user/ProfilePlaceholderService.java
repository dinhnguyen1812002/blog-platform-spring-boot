...
public class ProfilePlaceholderService {
    ...
    public String processPlaceholders(String rawMarkdown, User user) {
        ...
        if (processedContent.contains("{{post_count}}")) {
            processedContent = processedContent.replace("{{post_count}}", String.valueOf(postRepository.countByAuthor(user)));
        }
    ...
    private String generateLatestPostsHtml(User user) {
        // Get 5 latest posts
        List<Post> latestPosts = postRepository.findTop5ByAuthorOrderByCreatedAtDesc(user);
